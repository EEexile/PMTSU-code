import logging
import os
import contextlib

import numpy as np
import torch
from torch.utils.data import Dataset, DataLoader, random_split

logger = logging.getLogger(__name__)

def load_dataset(data_path, labels=None, min_length=3, max_length=None):
    sizes = []
    offsets = []
    emo_labels = []
    vad_labels = []
    
    npy_data = np.load(data_path + ".npy", mmap_mode='r')

    offset = 0
    skipped = 0

    if not os.path.exists(data_path + f".{labels}"):
        labels = None

    with open(data_path + ".lengths", "r") as len_f, open(
        data_path + f".{labels}", "r"
    ) if labels is not None else contextlib.ExitStack() as lbl_f:
        for line in len_f:
            length = int(line.rstrip())
            if labels is not None:
                lbl_line = next(lbl_f).rstrip().split()
                # 假设格式为: ID Emotion Valence Arousal Dominance
                if len(lbl_line) >= 5:
                    emo_lbl = lbl_line[1]
                    try:
                        vad_lbl = [float(lbl_line[2]), float(lbl_line[3]), float(lbl_line[4])]
                    except:
                        print("data process error")
                        vad_lbl = [0, 0, 0]
                
            if length >= min_length and (
                max_length is None or length <= max_length
            ):
                sizes.append(length)
                offsets.append(offset)
                if emo_lbl is not None:
                    emo_labels.append(emo_lbl)
                    vad_labels.append(vad_lbl)  # 存储VAD标签
            offset += length

    sizes = np.asarray(sizes)
    offsets = np.asarray(offsets)

    logger.info(f"loaded {len(offsets)}, skipped {skipped} samples")

    return npy_data, sizes, offsets, emo_labels, vad_labels

class SpeechDataset(Dataset):
    def __init__(
        self,
        feats,
        sizes,
        offsets,
        emo_labels=None,  # 情感标签
        vad_labels=None,   # VAD标签
        shuffle=True,
        sort_by_length=True,
    ):
        super().__init__()
        
        self.feats = feats
        self.sizes = sizes  # length of each sample
        self.offsets = offsets  # offset of each sample
        self.emo_labels = emo_labels
        self.vad_labels = vad_labels

        self.shuffle = shuffle
        self.sort_by_length = sort_by_length

    def __getitem__(self, index):
        offset = self.offsets[index]
        end = self.sizes[index] + offset
        feats = torch.from_numpy(self.feats[offset:end, :].copy()).float()

        res = {"id": index, "feats": feats}
        if self.emo_labels is not None:
            res["emo_target"] = self.emo_labels[index]  # 情感标签
        if self.vad_labels is not None:
            res["vad_target"] = self.vad_labels[index]  # VAD标签 [V, A, D]

        return res

    def __len__(self):
        return len(self.sizes)

    def collator(self, samples):
        if len(samples) == 0:
            return {}

        feats = [s["feats"] for s in samples]
        sizes = [s.shape[0] for s in feats]
        emo_labels = [s["emo_target"] for s in samples] if "emo_target" in samples[0] else None
        vad_labels = [s["vad_target"] for s in samples] if "vad_target" in samples[0] else None

        target_size = max(sizes)

        collated_feats = feats[0].new_zeros(
            len(feats), target_size, feats[0].size(-1)
        )

        padding_mask = torch.BoolTensor(torch.Size([len(feats), target_size])).fill_(False)
        for i, (feat, size) in enumerate(zip(feats, sizes)):
            collated_feats[i, :size] = feat
            padding_mask[i, size:] = True

        res = {
            "id": torch.LongTensor([s["id"] for s in samples]),
            "net_input": {
                "feats": collated_feats,
                "padding_mask": padding_mask
            },
            "labels": torch.tensor(emo_labels) if emo_labels is not None else None,
            "vad_labels": torch.tensor(vad_labels, dtype=torch.float) if vad_labels is not None else None
        }
        return res

    def num_tokens(self, index):
        return self.size(index)

    def size(self, index):
        return self.sizes[index]

def load_ssl_features(feature_path, emotion_dict, max_speech_seq_len=None):
    # 加载数据集并获取VAD标签
    data, sizes, offsets, emo_labels, vad_labels = load_dataset(
        feature_path, 
        labels='emo', 
        min_length=1, 
        max_length=max_speech_seq_len  # 确保这是整数或None
    )
    
    # 映射情感标签
    emo_labels = [emotion_dict[elem] for elem in emo_labels]
    

    
    num = len(emo_labels)
    iemocap_data = {
        "feats": data,
        "sizes": sizes,
        "offsets": offsets,
        "emo_labels": emo_labels,
        "vad_labels": vad_labels,  
        "num": num
    } 
    # print("VAD标签示例：", vad_labels[:3])
    return iemocap_data

def train_valid_test_iemocap_dataloader(
        data, 
        batch_size,
        test_start, 
        test_end,
        eval_is_test=False,
    ):
    feats = data['feats']
    sizes, offsets = data['sizes'], data['offsets']
    emo_labels = data['emo_labels']
    vad_labels = data['vad_labels']

    # 测试集分割
    test_sizes = sizes[test_start:test_end]
    test_offsets = offsets[test_start:test_end]
    test_emo_labels = [emo_labels[i] for i in range(test_start, test_end)]
    test_vad_labels = [vad_labels[i] for i in range(test_start, test_end)]

    test_offset_start = test_offsets[0]
    test_offset_end = test_offsets[-1] + test_sizes[-1]
    test_feats = feats[test_offset_start:test_offset_end, :]
    test_offsets = test_offsets - test_offset_start
    
    test_dataset = SpeechDataset(
        feats=test_feats,
        sizes=test_sizes, 
        offsets=test_offsets,
        emo_labels=test_emo_labels,
        vad_labels=test_vad_labels
    )

    # 训练集和验证集分割
    train_val_sizes = np.concatenate([sizes[:test_start], sizes[test_end:]])
    train_val_offsets = np.concatenate([np.array([0]), np.cumsum(train_val_sizes)[:-1]], dtype=np.int64)
    train_val_emo_labels = [emo_labels[i] for i in range(0, test_start)] + [emo_labels[i] for i in range(test_end, len(emo_labels))]
    train_val_vad_labels = [vad_labels[i] for i in range(0, test_start)] + [vad_labels[i] for i in range(test_end, len(vad_labels))]
    train_val_feats = np.concatenate([feats[:test_offset_start, :], feats[test_offset_end:, :]], axis=0)

    if eval_is_test:
        train_dataset = SpeechDataset(
            feats=train_val_feats, 
            sizes=train_val_sizes, 
            offsets=train_val_offsets,
            emo_labels=train_val_emo_labels,
            vad_labels=train_val_vad_labels
        )
        val_dataset = test_dataset
        train_loader = DataLoader(train_dataset, batch_size=batch_size, collate_fn=train_dataset.collator, 
                                num_workers=0, pin_memory=True, shuffle=True)
        val_loader = DataLoader(val_dataset, batch_size=batch_size, collate_fn=val_dataset.collator, 
                                num_workers=0, pin_memory=True, shuffle=False)
    
    else:
        train_val_nums = data['num'] - (test_end - test_start)
        train_nums = int(0.8 * train_val_nums)
        val_nums = train_val_nums - train_nums

        train_val_dataset = SpeechDataset(
            feats=train_val_feats, 
            sizes=train_val_sizes, 
            offsets=train_val_offsets,
            emo_labels=train_val_emo_labels,
            vad_labels=train_val_vad_labels
        )

        train_dataset, val_dataset = random_split(train_val_dataset, [train_nums, val_nums])

        train_loader = DataLoader(train_dataset, batch_size=batch_size, collate_fn=train_val_dataset.collator, 
                                num_workers=0, pin_memory=True, shuffle=True)
        val_loader = DataLoader(val_dataset, batch_size=batch_size, collate_fn=train_val_dataset.collator, 
                                num_workers=0, pin_memory=True, shuffle=False)
    
    test_loader = DataLoader(test_dataset, batch_size=batch_size, collate_fn=test_dataset.collator, 
                                num_workers=0, pin_memory=True, shuffle=False)

    return train_loader, val_loader, test_loader