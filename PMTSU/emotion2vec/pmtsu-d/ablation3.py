...
def train_iemocap(cfg: DictConfig):
    ...
    for fold in idx_sessions:  # extract the $fold$th as test set
        ...
        for epoch in range(cfg.optimization.epoch):
            ...
            ratio = min(epoch  / cfg.optimization.epoch, 1.0)
            ...
            logger.info(f"Epoch {epoch+1}: Task Weights: Emo={w_emotion:.2f}, VAD Total={total_vad_weight:.2f} (V={w_v:.2f}, A={w_a:.2f}, D={w_d:.2f})")
